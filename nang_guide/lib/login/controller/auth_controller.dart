...
import 'package:honbop_mate/core/dialog/welcome_dialog.dart';
...
import 'package:honbop_mate/login/views/google_signup_screen.dart';
...
class AuthController extends GetxController {
  ...
  Future<void> _handleLoginSuccess(
    ...
    if (onboardingSurveyCompleted) {
      // 완료된 유저는 홈으로
      print(
        'AuthController: Onboarding already completed. Navigating to Home.',
      );
      Get.offAllNamed(AppRoutes.HOME);
    } else {
      // 2. 미완료 유저는 신규 여부에 따라 웰컴 다이얼로그 후 설문 페이지로
      print(
        'AuthController: Onboarding NOT completed. Navigating to Pantry Onboarding.',
      );
      if (isNewUser) {
