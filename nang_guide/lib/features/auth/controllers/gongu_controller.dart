...
/// --------------------------------------------------
class GonguController extends GetxController {
  // GetxController는 상태 관리 + 로직처리 + 화면 전환을 담당. 이를 상속하겠다는건 GetxController 클래스의 모든 기능과 속성을 그대로 혹은 재정의해서 사용하겠다는 뜻
  /// Google OAuth 인증 처리 서비스
  late final GoogleAuthService
  _googleAuthService; // late는 나중에 초기화하겠다는 선언. 정리하면 처음 사용할 때 딱 한번만 값을 할당하고, 그 이후엔 절대 바꾸지 않겠다는 의미.
  late final AuthApiClient _authApiClient;
  late final GetStorage _storage;
  late final TokenService _tokenService;
  late final GonguService _gonguService;
  ...
  @override
  void onReady() {
    // onReady() 메서드는 화면이 다 준비되었으니 이제 본격적으로 프로그램을 돌려봐!라는 신호탄 역할. UI 렌더링이 완료된 후 호출됨. (전체적인 흐름 컨트롤러 생성 -> onInit() -> UI렌더링 -> onReady())
    super.onReady();
  ...
  Future<void> _checkLoginStatus() async {
    ...
    if (accessToken != null && refreshToken != null) {
      ...
      bool refreshed = await _tokenService
          .refreshToken(); // 결과는 bool(백엔드에서의 인증이 성공하면 토큰 재발급 받은 후 저장 및 true 반환 / 실패 또는 만료 시 false 반환 및 모든 토큰 제거)

      if (refreshed) {
        // 토큰 재발급 완료 시 홈화면으로 이동
        print(
          'AuthController: Auto-login successful via token refresh. Navigating to Home.',
        );
        Get.offAllNamed(AppRoutes.HOME);
      } else {
        // 토큰 재발급 실패(또는 만료) 시 다시 로그인 화면으로 이동
        print(
          'AuthController: Token refresh failed. Clearing tokens and navigating to Login Selection.',
        );
        await _tokenService.clearTokens();
        Get.offAllNamed(AppRoutes.LOGIN);
      }
    } else {
      // accessToken과 refreshToken이 없다면 로그인 화면으로 이동
      print(
        'AuthController: No existing tokens found. Navigating to Login Selection.',
      );
      Get.offAllNamed(AppRoutes.LOGIN);
    }
  }
  ...
  Future<void> completeGoogleRegistration(
    ...
    try {
      final authResponse = await _authApiClient.completeGoogleRegistration(
        registrationData,
      );

      if (authResponse.token != null) {
        print(
          'Frontend: Registration complete! JWT Token: ${authResponse.token}',
        );
  ...
  Future<void> logout() async {
    ...
    try {
      ...
      if (backendLoggedOut) {
        print(
          'AuthController: Backend logout successful, or tokens cleared due to 401. Proceeding with client-side logout.',
        );
  ...
  //   try {
  //     final success = await _authApiClient.changePassword(passwordData);

  //     if (success) {
  //       print('AuthController: Password changed. Forcing re-login.');

  //       // 서버 로그아웃 호출합니다
  //       await _tokenService.clearTokens();
  //       await _googleAuthService.signOut(); // Google 로그인 사용자일 경우
  //       Get.offAllNamed(AppRoutes.LOGIN);
  //     } else {
  ...
  //   try {
  //     final success = await _authApiClient.deleteAccount();

  //     if (success) {
  //       print('AuthController: Account deleted. Forcing re-login.');
